...
def insert():
    if request.method == "POST":
        ...
        cur = conn.cursor()
        cur.execute("INSERT INTO  sendingsms ( username,Date, Text ,Status ) VALUES (%s,%s,%s, %s)",
                    (username, formatted_date, sms_message, Status))
        conn.commit()
...
def outbox():
    cur = conn.cursor()
    cur.execute('SELECT * FROM sendingsms WHERE username = %s', (session['username']))
    data = cur.fetchall()
    cur.close()

    return render_template('layout2.html', sendingsms=data)
